...
export class Container 
{
  ...
  store: aptos_std$_.simple_map$_.SimpleMap;

  constructor(proto: any, public typeTag: TypeTag) {
    this.store = proto['store'] as aptos_std$_.simple_map$_.SimpleMap;
  }
...
export function create_resource_account$ (
  ...
  if (!$c.exists(new StructTag(new HexString("0x1"), "resource_account", "Container", []), $.copy(origin_addr))) {
    $c.move_to(new StructTag(new HexString("0x1"), "resource_account", "Container", []), origin, new Container({ store: aptos_std$_.simple_map$_.create$($c, [AtomicTypeTag.Address, new StructTag(new HexString("0x1"), "account", "SignerCapability", [])] as TypeTag[]) }, new StructTag(new HexString("0x1"), "resource_account", "Container", [])));
  }
  else{
  }
  ...
  aptos_std$_.simple_map$_.add$(container.store, $.copy(resource_addr), resource_signer_cap, $c, [AtomicTypeTag.Address, new StructTag(new HexString("0x1"), "account", "SignerCapability", [])] as TypeTag[]);
...
export function retrieve_resource_account_cap$ (
  ...
  if (!$c.exists(new StructTag(new HexString("0x1"), "resource_account", "Container", []), $.copy(source_addr))) {
    throw $.abortCode(std$_.error$_.not_found$(ECONTAINER_NOT_PUBLISHED, $c));
  }
  resource_addr = std$_.signer$_.address_of$(resource, $c);
  container = $c.borrow_global_mut<Container>(new StructTag(new HexString("0x1"), "resource_account", "Container", []), $.copy(source_addr));
  [_resource_addr, signer_cap] = aptos_std$_.simple_map$_.remove$(container.store, resource_addr, $c, [AtomicTypeTag.Address, new StructTag(new HexString("0x1"), "account", "SignerCapability", [])] as TypeTag[]);
  [resource_signer_cap, empty_container] = [signer_cap, aptos_std$_.simple_map$_.length$(container.store, $c, [AtomicTypeTag.Address, new StructTag(new HexString("0x1"), "account", "SignerCapability", [])] as TypeTag[]).eq(u64("0"))];
  if (empty_container) {
    ...
    aptos_std$_.simple_map$_.destroy_empty$(store, $c, [AtomicTypeTag.Address, new StructTag(new HexString("0x1"), "account", "SignerCapability", [])] as TypeTag[]);
  }
  else{
  }
